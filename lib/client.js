    ...
    function BrowserSection() {
      ...
      const doDoctor = async () => {
        ...
        try {
          ...
          if (r && r.ok) {
            setDoctorOut(r.detail || '(无输出)')
            setStatus({ text: r.error ? `诊断发现异常：${r.error}` : '诊断完成', kind: r.error ? 'err' : 'ok' })
          } else setStatus({ text: (r && r.error) || '诊断失败', kind: 'err' })
          await refresh()
        } catch (e) { setStatus({ text: String((e && e.message) || e), kind: 'err' }) }
        setBusy(false)
      }
      ...
      return h('div', { className: 'bhn-wrap' },
        ...
        h('div', { className: 'bhn-card' },
          h('div', { className: 'bhn-row' },
            ...
            h('span', { className: 'bhn-hint' }, '上游项目 browser-use/browser-use + browser-harness CLI'),
          ),
          updates === null
            ...
            : h(React.Fragment, null,
                h('dl', { className: 'bhn-kv' },
                  h('dt', null, '项目（GitHub）'),
                  h('dd', null, `browser-use/browser-use${updates.project && updates.project.latest ? ` 最新 v${updates.project.latest}` : '（无法获取）'}`),
                  h('dt', null, '当前对应'),
